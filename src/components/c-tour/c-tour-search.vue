    ...
    <view>
        ...
        <view v-for="(item, index) in showData" :key="index" class="building-item">
            <view style="height: 100%; display: flex">
                ...
                <navigator
                    class="text"
                    :url="'polyline?latitude=' + item.latitude + '&longitude=' + item.longitude"
                >
...
export default {
    ...
    methods: {
        bindSearchInput: function (e) {
            ...
            const searchdata = this.buildlData;
            if (e.detail.cursor >= this.cursor) {
                //输入文字
                const inputData = e.detail.value.replace(/(^\s*)|(\s*$)/g, "");

                if (inputData) {
                    let z = 0,
                        x = 100;
                    for (const b in searchdata) {
                        for (const i in searchdata[b].data) {
                            if (
                                searchdata[b].data[i].name.indexOf(inputData) != -1 ||
                                (searchdata[b].data[i].floor &&
                                    searchdata[b].data[i].floor.indexOf(inputData) != -1)
                            ) {
                                const build = searchdata[b].data[i];
                                build.tid = b;
                                build.bid = i;
                                z = z + 1;
                                build.index = z;
                    ...
                    for (let i = 0; i < showData.length; i++) {
                        for (let j = 0; j < showData.length - i - 1; j++) {
                            if (showData[j].index > showData[j + 1].index) {
                                const temp = showData[j];
